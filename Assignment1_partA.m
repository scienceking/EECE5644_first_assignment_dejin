%% This code is for the first assignment of EECE 5644, the first question: part A.
clear;
clc;
...
% Compute likelihood ratio for samples
likelihood_ratio = mvnpdf(X, m1', C1) ./ mvnpdf(X, m0', C0);
...
FNR_list = zeros(length(gamma_values), 1); % List to store False Negative Rates
...
for i = 1:length(gamma_values)
    ...
    % Store TPR, FPR, FNR and error probability
    TPR_list(i) = TPR;
    FPR_list(i) = FPR;
    FNR_list(i) = FNR;
...
decision_fixed = likelihood_ratio > gamma_fixed;
...
FNR_fixed = FN_fixed / (FN_fixed + TP_fixed); % False Negative Rate

error_prob_fixed = FPR_fixed * P_L0 + FNR_fixed * P_L1; % Error probability for gamma = 0.35 / 0.65

% Output the error probability and coordinates for gamma = 0.35 / 0.65
fprintf('Error Probability for gamma = 0.35 / 0.65: %.4f\n', error_prob_fixed);
...
title('ROC Curve (Gamma from 0 to Infinity)');
grid on;
